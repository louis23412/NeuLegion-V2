...
| `features.js` | LOCKED-invariant | finance | **the causal signal family** (round 23, N1): exact reference vectors for all eight features — `momentum` (trailing sum), `fracDiffAt`/`fracMomentum` (`d=1` reduces to the log first/second difference), `volRegime` (0 on equal-length windows, -1 on a flat short window), `momentumAgreement` (±1 when every lens agrees), `rangeLocation` (±0.5 at the range edges), `volumeImbalance` (exact short/long mean ratio), `autocorr1` (-1 on a zero-mean alternating series), `acceleration` — plus the exact `(raw-mean)/σ` z-score, the `minObs`/zero-variance abstain, `clampPosition` bounds/oddness, and a causality invariance check (a position at `t` is unchanged when every value after `t` is altered). `SIGNAL_CANDIDATES` is frozen, uniquely idd, and every `signal(view, test)` returns `|test|` positions in `[-1, 1]`, abstaining on a returns-only/empty view. **Round 29 → 30 (P3)** adds the **opt-in** short-horizon reversal family (`reversal`/`reversalWindow`/`reversalVol`/`crossSectionalReversal`, `REVERSAL_CANDIDATES`) — causal through the new index-aligned `view.panel` and abstaining (never throwing) without one. **Round 30 (C-SIGUP / C-REGIME, gate G-H)** adds the **opt-in** momentum-upgrade family (`volScaledMomentum`, `blendedMomentum`, `networkMomentum`, `regimeGatedMomentum`, `SIGUP_CANDIDATES`) — vol-scaled, multi-horizon blended, strictly-lagged network (panel-scoped, `BUGS.md` #70) and a causal crash-gated momentum, each through the same pure point-in-time + causal z-score + `clampPosition` pipeline with an exact reference vector, the strict-lag/panel contract and a family-wide causality check (`analysis.test.js` §G-H) |
...
| `benchmark.js` | LOCKED-invariant | finance | **the P1 model-class benchmark** (round 29 → 30): base rate / ridge (closed form) / one-hidden-layer tanh MLP (seeded SGD) / a pluggable pretrained-TSFM arm, each a `{fit(X,y), predictProb(x)}` forecaster over the **same causal feature matrix** the bare path feeds the controller. Pure + deterministic (only RNG is a seeded `mulberry32`), so a benchmark arm is CRN-consistent. Proved: exact standardiser (zero-mean/unit-std, constant-column fallback), ridge closed form vs a hand solve, seeded MLP determinism, base rate = training prior, `BENCHMARK_KINDS` are the expected kinds, a perfect classifier beats the base rate on Brier. Wired opt-in by `analyze.js` (`--variants=bench-base-rate,bench-linear,bench-mlp`; a `kind:"benchmark"` variant dispatched to its own factory under either `--model`, not a `model` value), never in the default roster, grouped into the **calibration** forecast kind | `analysis.test.js`, `analyze.test.js` |
| `carry.js` | LOCKED-invariant | finance | **the funding/basis CARRY sleeve** (round 29 → 30, P4): audit a Binance funding JSONL on its own grid and build the delta-neutral (short perp / long spot) return on the candle bar grid — a structurally independent return source, since a positive rate means longs pay shorts so the book **earns** `+rate` per period (`signFlip` inverts). Pure + network-free (the caller fetches via `funding_fetcher.js`). Proved (§P4): `parseFundingJsonl`/`auditFundingSeries` on a fixture (off-grid budget, exact-zero periods, `unclosed` = timestamp in the future); `carryReturns` is the receipt and `signFlip` inverts it; `carryOnBarGrid` is causal, sums to the rate, and normalises **both** axes to epoch ms (an ISO-string compare is silently all-zero — `BUGS.md` #66); `carryPanelStream` tiles the fold test bars in the pooled price index space; `correlation`/`pooledCarry` exact | `analysis.test.js` (§P4) |
...
### Hyperplane LSH recall (`lsh.test.js`, 75 checks)
...
### Sample-uniqueness loss weighting (`sample_weights.test.js`, 57 checks)
