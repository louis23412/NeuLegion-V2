...
export const ANALYSIS_MODULES = Object.freeze({
    'performance.js': [
        ...
        // The Euler–Mascheroni constant the DSR expected-max term reads (a public
        // constant of the module, like the other pinned defaults).
        'EULER_MASCHERONI',
    ],
    'splits.js': [
    ...
    'backtest.js': [
        ...
        // The per-fold aggregation, the worker path and the re-export the merge/parallel
        // reports use.
    ...
    'benchmark.js': [
        'fitStandardiser', 'applyStandardiser', 'fitRidge', 'predictRidge', 'fitMLP',
        'predictMLP', 'fitBaseRate', 'predictBaseRate', 'BENCHMARK_KINDS', 'makeBenchmarkForecaster',
    ],
    'carry.js': [
        'FUNDING_GRID_MS', 'FUNDING_PER_YEAR', 'parseFundingJsonl', 'auditFundingSeries',
        'auditFundingProblems', 'carryPerPeriod', 'carryReturns', 'carryOnBarGrid',
        'carryPanelStream', 'correlation', 'pooledCarry',
    ],
    'dependence.js': [
    ...
    'features.js': [
        ...
        // Round 29 → 30 (P3): the opt-in short-horizon reversal family.
...
export const ANALYSIS_REGISTRY = Object.freeze({
    ...
    'benchmark.js': {
        status: LOCK_LEVELS.INVARIANT,
        domain: 'finance',
        citations: ['brier1950', 'murphy1973', 'gneitingraftery2007', 'diebold1995', 'hansenlundenason2011', 'leprado2018afml'],
        proves: ['analysis.test.js', 'analyze.test.js'],
        note: 'The P1 model-class benchmark (round 29 -> 30): base rate / ridge (closed form) / one-hidden-layer tanh MLP (seeded SGD) / a pluggable pretrained-TSFM arm, each a `{ fit(X, y), predictProb(x) }` forecaster over the SAME causal feature matrix the bare path feeds the controller. Pure and deterministic (no I/O; the only RNG is a seeded mulberry32), so a benchmark arm is CRN-consistent with every other arm. Why it exists: the controller has negative forecast skill and the model-class literature says a tiny permutation-invariant transformer trained on ~60 bars is the wrong forecaster for this data (DLinear/TSMixer dominate), so P1 settles "is the forecaster or the target the problem?" on THIS data. Proved (analysis.test.js): the standardiser is exact (zero mean / unit std, constant-column fallback), the ridge closed form matches a hand-computed solve, the MLP is deterministic under a seed, the base rate equals the training prior, BENCHMARK_KINDS are the expected kinds, and a perfect classifier scores a strictly lower Brier than the base rate. Wired opt-in by analyze.js (`--variants=bench-base-rate,bench-linear,bench-mlp`; a `kind: "benchmark"` variant dispatched to its own factory under either `--model`, not a `model` value), never in the default roster (adding candidates re-deflates every Sharpe), and grouped into the calibration MCS kind (`forecastKindOf`). Readout: RUN-ANALYSIS.md §16.2.',
    },
    'carry.js': {
        status: LOCK_LEVELS.INVARIANT,
        domain: 'finance',
        citations: ['kish1965', 'grinold1989', 'leprado2018afml', 'finval2609'],
        proves: ['analysis.test.js'],
        note: 'The funding/basis CARRY sleeve (round 29 -> 30, P4): audit a Binance funding JSONL on its own grid and build the delta-neutral (short perp / long spot) return series on the candle bar grid. Why it exists: the shipped basket has eight price streams that are one factor wearing eight hats (design effect ~5.2x, ~1.3 effective streams of 9), so the only way to buy real independence is a new data source, and the perpetual funding rate is structurally not the price return (a positive rate means longs pay shorts, so the delta-neutral book EARNS +rate per period; `signFlip` inverts). Pure and network-free: the caller fetches the rows (funding_fetcher.js) and hands them in. Proved (analysis.test.js): parseFundingJsonl / auditFundingSeries on a fixture (off-grid-step budget, exact-zero periods, unclosed = timestamp in the future); carryReturns is the receipt and signFlip inverts it; carryOnBarGrid is causal (nothing before the first closed period), sums to the rate, and normalises BOTH the bar axis and the funding rows to epoch ms (an ISO-string compare is silently all-zero -- BUGS.md #66); carryPanelStream tiles the fold test bars in the pooled price index space; correlation/pooledCarry are the exact equal-weight statistics. Measured readout: RUN-ANALYSIS.md §16.5.',
    },
    'holding.js': {
        status: LOCK_LEVELS.INVARIANT,
        domain: 'finance',
...
export const SUPPORT_MODULES = Object.freeze({
    ...
    'sanitize.js': [
        ...
        // The numeric-field allow-lists the two sanitizers read (public constants,
        // siblings of STRUCTURE_CONFIG_KEYS).
        'SIGNAL_NUMERIC_FIELDS', 'CONSENSUS_NUMERIC_FIELDS',
    ],
    'rng.js': [
    ...
    'analyze.js': [
        // A CURATED subset of the driver's public API — the pure helpers plus the A/B
        // entry points and the default roster constants — NOT an exhaustive export list.
        // `analyze.js` is a driver (CLI, run-directory I/O, the parallel dispatcher), so
        // its roster/label/reversal constants, its CLI printers (`listVariants`,
        // `formatVariantList`), its `replicateAnalysis`/`inertReasonFor` helpers and the
        // async dispatchers (`evaluateABAsync`, `makeNodeFoldDispatcher`) are intentionally
        // outside this contract. Do not treat an unlisted `analyze.js` export as a gap.
...
export const CONTROLLER_REGISTRY = Object.freeze({
    controllerDatabase: {
        ...
        note: 'Opens the per-controller SQLite DB. Promoted NEEDS-LOCAL-RUN -> LOCKED-invariant after the green local `npm test` (BUGS.md #18): the golden/core mirrors drive this path on the real better-sqlite3 driver, and `golden.test.js` pins the bounded reload gap (<1e-4) plus the reloaded memory structure. R27-4: `global_stats.value` is declared NUMERIC (it stores a float `brier_sum`; SQLite INTEGER affinity stored it losslessly but the schema claimed a type the data did not have).',
    },
    controllerAccuracy: {
        status: LOCK_LEVELS.INVARIANT,
        domain: 'ensemble',
