...
warming a per-stream snapshot) live in `../docs/METHOD.md`, and the round-27 plan
(per-candidate liveness, a causal streaming sample-uniqueness mechanism, the two
unrun experiments) is in `../docs/PLAN-round27.md` — **implemented
(R27-1…R27-6, R27-8, R27-9); the two experiments have not yet been run.** The
round-27 runs are in `RUN-ANALYSIS.md` §13 and the round-28 plan in
`../docs/PLAN-round28.md` — **implemented, and its three operator runs are done (readout
`RUN-ANALYSIS.md` §15)**: it fixed the reading layer (the sizing
arithmetic, the gate-vs-`DESIGN.md`-§6.1 drift, the measured inert reasons), re-armed
sample-uniqueness weighting with a *measured* causal span and a mean-1 emitted stream (plus a
scale-control arm), measured the `pca-hash` retrieval liveness, and recorded the
label-policy/`sig-accel` decisions (`RUN-ANALYSIS.md` §14; `BUGS.md` #53–#59). The runs then
read out: the weighting mechanism is live and mildly positive but sub-threshold, the label
policy's `--test=10` effect did not replicate across the retrain cadence, the round-27
`sig-accel` promotion was a roster-size/multiplicity artefact (`K = 3` → `K = 12`), and ~100 %
of the measured edge in these arms is net-exposure × market (`BUGS.md` #60/#61/#62, all
reported-not-fixed). No code changed after the implementation, so the test ledger is unchanged. The
**round-29 plan** is `../docs/PLAN-round29.md` — **implemented and measured (P1–P4; P5 deferred
with its gate open)**: it started from
the fact that the controller has *negative* forecast skill and that ~100 % of every positive-Sharpe
arm is market exposure, re-sourced the edge from a shorter-horizon **reversal** and/or a
**funding/basis carry** stream (new data), benchmarked the forecaster against linear/MLP/pretrained
models, made the verdict configuration-robust, and added one bounded probe of the design's own
question (P7: does the controller's ensemble **size** matter?). Start at the consolidated index
`../docs/research/round29-README.md` (decision table, conflict register C1–C10, measured checks
MC1–MC4; machine-readable mirror `round29-registry.json`; **§8 = the implementation verdicts**) and
the five `../docs/research/round29-*.md` notes. **Two measured coherence checks** this cycle: the 1h
coherence check (one factor = 79.4 % of the 8-stream covariance, 1-factor residual 20.8 % (rank IC
−0.050), 1h lag-1 autocorrelation −0.013) and the **adjusted-DSR surface** (the two highest-Sharpe
arms fail on *exactly one* hurdle — the design-effect-adjusted DSR — while the Step-2 baseline
passes at Sharpe 0.8978; this corrected `PLAN-round29.md` §1.5). The implementation's readouts are
`../docs/RUN-ANALYSIS.md` §16 (with the execution log `../docs/round29-IMPLEMENTATION.md`): **P1** →
G-A negative, **P2** → configuration-robust + exposure-matched gate, **P3** → the 15m reversal is
real but taker-cost-inaccessible, **P4** → the carry sleeve is a correlation/effective-streams
independence purchase that does not cross the DSR surface, **P5** deferred. The **round-30
implementation** (`../docs/PLAN-round30.md`, milestone `../docs/MILESTONES.md`
M7.2) prunes the A/B roster to the branches that can clear the gate (M1–M3 **implemented**:
`src/lineage.js` + `analyze.js`'s pruned `ALL_VARIANTS`), registers the family trees
(`../docs/LINEAGE.md` + `../docs/lineage.json`) and the dropped hypotheses (`../docs/DROPPED.md`),
and pulls the research on what proved working (`../docs/research/round30-winning-mechanisms.md`);
the acceptance batch it is built from is `../docs/RUN-ANALYSIS.md` §17. **A drop is a roster label,
never a code deletion** — see the note below. This
...
## Round 30 — pruned roster + lineage (implemented)

The round-30 plan reduces the **default A/B roster** (`analyze.js#ALL_VARIANTS`) from 14 arms to
`{baseline, sig-momentum, sig-accel}` (M1 **done**): `ALL_VARIANTS` is filtered by
`./lineage.js#DEFAULT_ROSTER_IDS`, the roster is content-hashed by `analyze.js#rosterSnapshot()`,
and the register contract (`./lineage.js`) is pinned by the `analyze.test.js` `§A2b` checks — every
resolvable id registered, the `uncovered` set empty, the snapshot hash stable, and every dropped
branch absent from the roster yet still resolvable. The dropped *hypotheses* are recorded with
their measurements in `../docs/DROPPED.md`; the register `../docs/LINEAGE.md` (mirrored by
`../docs/lineage.json` + `./lineage.js`) carries the ids,
...
| `pca-hash` | DROPPED (behaviourally inert) | `hivemind/memory/binarypc.js` + `bitweight.js` + the `lsh` `_pcaHashConfig` path; `binarypc.test.js`, `bitweight.test.js`, `lsh.test.js` |
| `sig-frac-momentum`, `sig-vol-regime`, `sig-autocorr`, `sig-volume` | DROPPED (worst / negative / regime-dependent / flat) | `analysis/features.js` signal branches — kept in `RESOLVABLE_VARIANTS` for by-id reproducibility |
| `bench-mlp` | DROPPED (worst forecaster) | `analysis/benchmark.js` `fitMLP` — kept in the opt-in `BENCHMARK_VARIANTS` roster |

`multiprobe`/`querymod` are **PARK** (not dropped — they act on the broadcast path the scored model
never reads, `R27-2`), as are `sig-range`/`sig-agreement` (positive, with real cross-sectional
content) and the `NL-REV` reversal family (a taker-cost rejection).

**The pre-registered momentum upgrades (M7, gate G-H).** `analysis/features.js` gains the round-30
opt-in `SIGUP_CANDIDATES` family — `sig-vol-momentum`, `sig-blend-momentum`,
`sig-network-momentum` and `sig-regime-momentum` (vol-scaled / multi-horizon blended /
network lead-lag / causal crash-gated momentum, `../docs/research/round30-winning-mechanisms.md`
§1.2). Same pure point-in-time + causal z-score + `clampPosition` pipeline, resolved only by id
(`--variants=sig-vol-momentum,...`) so the default roster, `K` and every golden are untouched;
`sig-network-momentum` reads `view.panel` and is marked `crossSectional` (`BUGS.md` #70). They are
...
| `analysis/features.js` | `DEFAULT_POSITION`, `clampPosition`, `momentum`, `fracDiffAt`, `fracMomentum`, `volRegime`, `momentumAgreement`, `rangeLocation`, `volumeImbalance`, `autocorr1`, `acceleration`, `causalZScore`, `positionAt`, `signalForCandidate`, `SIGNAL_CANDIDATES`, `reversal`, `reversalWindow`, `reversalVol`, `crossSectionalReversal`, `REVERSAL_CANDIDATES`, `volScaledMomentum`, `blendedMomentum`, `networkMomentum`, `regimeGatedMomentum`, `SIGUP_CANDIDATES` | **the causal signal family** (round 23, N1): 8 pure point-in-time features (momentum, d=0.4 fractional-diff momentum, realized-vol regime, multi-horizon momentum agreement, range location, volume/turnover imbalance, lag-1 autocorrelation, acceleration), each reduced to a position by one causal z-score → clamp pipeline. They are the A/B's real candidates on the same family-wise gate as the mechanism flags, so this family's gate has historically been quoted at K = 15 (the round-30 default roster prunes it to `{baseline, sig-momentum, sig-accel}` → `K = 3`, every other branch staying resolvable by id). Two OPT-IN families share the pipeline: the round-29/30 short-horizon reversal (`REVERSAL_VARIANTS`) and the round-30 pre-registered momentum upgrades (`SIGUP_CANDIDATES`, gate G-H: vol-scaled, multi-horizon blended, network/lead-lag and regime-gated momentum) | `analysis.test.js` (§AC, §P3, §G-H) |
...
| `analysis/forecast.js` | `forecastPairs`, `brierScore`, `logScore`, `brierDecomposition`, `brierLosses`, `bootstrapMeans`, `dieboldMariano`, `modelConfidenceSet`, `forecastComparison`, `formatForecast` | **forecast comparison** (round 26, R26-14): the family scored as forecasters — proper scores (Brier + Murphy reliability/resolution/uncertainty, log score; Gneiting & Raftery 2007), the block-bootstrapped Diebold–Mariano test on per-bar Brier-loss differentials (Diebold & Mariano 1995), and the Hansen–Lunde–Nason Model Confidence Set at 90/95% (the families that cannot be distinguished from the best). Pure post-processing of the journaled confidence; on by default in `analyze` (`--forecast=0` disables). R27-5: scored WITHIN a forecast kind (`forecastKindOf`) — the controller family's journaled confidence is `confidenceFromProb(prob)` (a probability), a signal's is a normalised z-score, so each kind gets its own MCS and the DM test runs only inside the baseline's kind (a cross-kind candidate carries `dm.available=false` with a reason) | `analysis.test.js` (§AI) |
| `analysis/decision.js` | `foldConcentration`, `confidencePersistence`, `nextRunPlan`, `decisionReport`, `formatDecision`, `promotionAcrossCadences`, `defaultCatastrophic` | **the decision-grade report** (round 26, R26-8): composes the run's already-computed blocks into the six questions the next cycle asks (training / edge / concentration / economics / family / nextRun), each field a value or an explicit `available:false` + reason, and adds the concentration readout (leave-one-fold-out Sharpe range + per-fold marginal contribution), the raw-confidence half-life and the next-run sizing knobs. **Round 28** stops the sizing block mixing scales: `cheapestFlip`'s `magnitude` branch and `pairedUnitsNeeded` read the PAIRED SE and the test's ONE-SIDED cluster-t reference, `nextRunPlan.scales` names which fields are single-series and which are paired, and the cross-scale `needed.mde95Dependent` target is replaced by `neededForObservedPower80` (`BUGS.md` #56) | `analysis.test.js` (§AK, §AM), `analyze.test.js` |
...
## Round 29 — model-class benchmark, the carry sleeve, and configuration robustness

Round 29 (readout `../docs/RUN-ANALYSIS.md` §16; execution log
`../docs/round29-IMPLEMENTATION.md`) added two pure analysis modules
(`analysis/benchmark.js`, `analysis/carry.js`), a new fetch library (`funding_fetcher.js`), two new
data manifests, and two
decision-procedure changes. **P1–P4 are implemented and measured; P5 (continuous TTA) is deferred
with its gate open** — `hivemind/training/gradients.js` and the golden-pinned hot path are
...
`CANDLE_FILES_15M` / `QUARTER_HOUR_MS` (`data/candles_<sym>_15m.jsonl`: 8 symbols × 79 999 bars,
2024-06 → 2026-09); the funding basket is `FUNDING_MANIFEST` / `FUNDING_FILES`
(`data/funding_<sym>_8h.jsonl`: 8 symbols, **57 939** rows, 2019-09 → 2026-09). They are **separate
manifests**, not extra entries in the 1h `CANDLE_MANIFEST`, so the 1h basket's invariants, the
interval-blind tests and `--symbols=all` are untouched (the 15m files are addressed by path via
`--files=`). Both were audited cleanly before any run (`PLAN-round29.md` §8 step-0 hard gate);
funding's only imperfection is SOL's 98 off-grid steps (1.47 %, inside the 2 % budget).

**Decision-procedure changes (no golden moves; rationale `../docs/METHOD.md` §10/§11).**
`analysis/decision.js` gains `promotionAcrossCadences` + `defaultCatastrophic` (majority-pass with a
catastrophic veto; every level field carries its cadence), `analysis/walkforward.js` gains
`exposureDeadZone` / `exposureMatchedPair` / `restateReportAtCadence` (cross-family statements only
at matched in-market share; the target share is read from the restatement, `BUGS.md` #63), and
`analysis/features.js` gains the opt-in 15m reversal family (`REVERSAL_VARIANTS`) plus the
cross-sectional `view.panel`. `analyze.js` gains `--carry-files` / `extraPanelStreams` with a
per-stream length check and a constant-stream guard (`panelMismatchReason: 'length' | 'degenerate'`,
`BUGS.md` #64/#66) and reports `dependenceWithoutExtras`. It also **wires the P2 gate into the
driver** as opt-in passes: `--cadences=a,b,c` restates every active candidate on each fold grid and
writes the majority-pass + catastrophic-veto verdict to `report.configurationRobust`, and
`--exposure-match` writes the matched-exposure comparison to `report.exposureMatched`. Both are
default-off pure post-processing of the journaled confidence, so the default report is
byte-identical. P5's design/cost wall are recorded in
`../docs/RUN-ANALYSIS.md` §16.6 and `../docs/TODO.md` 96.
...
projections, multi-set / multi-table). `test/browser/entries/lsh.test.js` (75
...
unchanged. **Round 27 (R27-3)** re-scopes the mechanism: on the shipped `optimistic`
labeler the labels do not overlap (every label spans one bar), so average uniqueness is
exactly 1 and the weighting is mathematically inert there (closed `not-applicable`); it
ships as an opt-in `causal-window` modifier of the now-reachable `triple` label, using
the pure `causalWindowWeight(windowSpans, newSpan, config)` ring, with its weight
distribution and ESS reported. `sample_weights.test.js` (57 checks) proves the pure math (bit-for-bit
against `analysis/uniqueness.js`) and that the train step is exactly linear in the
weight. **Round 28 (`BUGS.md` #54/#58):** the "labels do not overlap" premise above was the
**#49 false diagnostic** — the fixed `heldBars` is mean ≈8 / max 54, so the `optimistic`
labels *do* overlap and it was the ring's *assumed span horizon* (configured to 1) that
made every weight exactly 1. The span is now a **causal** estimate (EMA α 0.1 of the
realized holding periods of trades already drained, or an explicit
...
| \`analyze.js\` | \`npm run analyze\` — the walk-forward **feature A/B driver** (ROADMAP P2-1): the default-off variant family plus, after round 23, the 8 causal signal candidates (a K = 15 family-wise gate historically; the round-30 default roster is K = 3); \`promoteDecision\` + \`walkForwardSearch\`, a run report. The default model is the **shipped \`HiveMindController\`** (\`makeControllerModelFactory\`: real OHLCV, prequential per fold, documented \`prob -> position\` policy, abstains on the \`-1\` sentinel or a throw); \`--model=bare\` reproduces the round-22 proxy, \`--symbols=a,b|all\` + \`--files\` pool streams (all 8 audited symbols), \`--probe\`/\`--audit-probes\`/`--variants\`/\`--bars\`/\`--train\`/\`--test\`/\`--seed\` tune the run. Round 24b (premium hardening after the first complete smoke run): `--reuse-base` reuses the scored pass as the audit base pass (one fewer refit per fold, verdict-identical, recorded in the report as `baseReused`), `--cost-bps` is threaded to the scoring, `folds.jsonl` lines carry `probeIndex` + `reused` (so the journal is readable offline), and the summary states the run-level power verdict. Round 24 (run integrity, all arithmetic-free): \`modelRetention\` (factory default \`keep\`, CLI default \`discard\`) reclaims each fit's state dir; a run writes \`run.json\` / \`report.json\` / \`partial-report.json\` / \`progress.json\` / \`folds.jsonl\` / \`run.log\`; the report carries a machine-readable \`audit\` block (\`clean\`/\`reachable\`/\`reachableFolds\`/\`probes\`); flags \`--keep-models\` / \`--reachable\` / \`--fold-log\` / \`--progress-ms\` / \`--help\`. Round 26 adds the corrected roster + flags: `--concurrency` (worker-backed folds, byte-identical journal), `--forecast`/`--decision`/`--crn`/`--interval`/`--select-streams`/`--turnover-sweep`/`--label-policy`/`--label-policies`/`--save-interval`/`--seeds`, and `run.json`/`report.json` echo the `gate`/`gateOptions`/`trials` the DSRs were deflated by. Note (round 26, `BUGS.md` #43/#44): the `sample-weights`/`multi-probe`/`query-mod` roster entries are inert on the controller path, so their A/B rows are duplicates of the baseline. Round 27 fixes this at the reading layer: every candidate carries a `liveness` certificate (live/inert/duplicate-of:<id>/not-applicable/skipped), an inactive candidate is excluded from K and the family-wise search with exactly one reason (`trialsRoster = trials + trialsInactive`); a candidate taxonomy (`appliesTo`, printed by `--list-variants`) marks `multi-probe`/`query-mod` not-applicable on the controller while `pca-hash` stays live; `sample-weights` leaves the default roster but stays resolvable; `requireReachable` defaults true (`--reachable=0` opts out); `--audit-probes` defaults 1; `--min-training-steps` sets the `underTrainedFolds` floor; the forecast panel is scored per kind; and `streamLabel` is the symbol/basename, never the operator's path. Round 29 adds the benchmark variant dispatch and the opt-in `--cadences=a,b,c` (→ `report.configurationRobust`) and `--exposure-match` (→ `report.exposureMatched`) passes (default-off, pure post-processing, so the default report is byte-identical). | \`analyze.test.js\` (269) |
...
list is `CANDLE_MANIFEST` in `candles_audit.js`. **Round 29 added two further
baskets** (also `candles_audit.js`, both audited by `candles.test.js`): a 15m
basket `data/candles_<sym>_15m.jsonl` (**8 symbols × 79,999 bars**, 2024-06 →
2026-09, the round-29 P3 reversal dataset) and a funding basket
`data/funding_<sym>_8h.jsonl` (**57,939 rows**, 2019-09 → 2026-09, the P4 carry
dataset). `candle_fetcher.js` is the
network-free fetch/merge/gap library (tested by `fetcher.test.js`, 111 checks);
`fetch_candles.js` is the single-symbol CLI, `funding_fetcher.js` the funding
fetch/merge library, and `update_candles_basket.js` the
