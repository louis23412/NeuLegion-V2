...
class IndicatorProcessor {
  compute(candles) {
    ...
    const validCandles = candles.filter(c =>
      ...
      isValidNumber(c.volume) && c.volume >= 0 && isValidTimestamp(c.timestamp)
    );
  ...
  #computeRSI(values, period) {
    ...
    for (let i = period, j = 0; j < rsi.length; i++, j++) {
      ...
      if (avgLoss <= 0) {
        // No losses in the window: RSI saturates at 100 (50 if nothing moved).
        rsi[j] = avgGain <= 0 ? 50 : 100;
      } else {
  ...
  #computeBollingerBands(values, period, stdDevMultiplier) {
    if (!Array.isArray(values) || values.length < period || !values.every(isValidNumber)) {
      return new Array(Math.max(0, values.length - period + 1)).fill({ middle: 0, upper: 0, lower: 0 });
    }
