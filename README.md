...
[`docs/ROADMAP.md`](docs/ROADMAP.md); the A/B run forensics (all four power-run
attempts), the round-24/24b run-integrity work (checkpointing, state reclamation,
a liveness heartbeat, the audit block, `--reuse-base`), the **recorded N3 verdict**
(nothing promotes) and the
**round-26 controller/A-B fidelity sweep** (which found that the A/B fed the
controller the whole candle prefix where production feeds a fixed window, so its
baseline rows were not the shipped model — `BUGS.md` #33; its second pass added the
...
[`docs/OPTIMIZATION.md`](docs/OPTIMIZATION.md). **The current verdict is the
corrected re-run `20260922T204248-seed1` (`RUN-ANALYSIS.md` §10): all 14 candidates
keep-off, SPA p = 0.4731, nothing promotes at any cost level, baseline Sharpe
-0.1147 with negative skill — §5's attempt-3 baseline (+0.4387) predates the
...
[`docs/PLAN-round28.md`](docs/PLAN-round28.md) — **implemented and run** (forensics
`RUN-ANALYSIS.md` §15). **Round 29 is implemented and measured** ([`docs/PLAN-round29.md`](docs/PLAN-round29.md);
readouts `RUN-ANALYSIS.md` §16, execution log
[`docs/round29-IMPLEMENTATION.md`](docs/round29-IMPLEMENTATION.md)). The round-28 runs' honest
conclusion was that the bot has no demonstrated *residual* edge (the controller has negative
forecast skill and ~100 % of every positive-Sharpe arm is market exposure), so round 29 re-sourced
the edge from a shorter-horizon **reversal** (`2608.21888`) and a **funding/basis carry** stream,
benchmarked the forecaster against base-rate/linear/MLP models, and made the verdict
configuration-robust — grounded in four new
[`docs/research/round29-*.md`](docs/research/) notes. **Outcome:** the **architecture is not the
constraint** (P1's benchmark — no model class has positive Brier skill vs the base rate, so P6/P7
are closed), the **15m reversal is real but economically inaccessible** at a 5–10 bps taker cost
(P3), the **carry sleeve is a genuinely low-correlation return source** (0.003 vs the price basket;
effective streams 1.25 → 1.54 of 9) that does **not** cross the DSR gate (P4), and **P5 (continuous
TTA) is deferred with its gate open**. The
...
The A/B verdict run and the round-30 operator set (P1 benchmark, P3 reversal, P4 carry sleeve,
the pruned verdict run, the G-F seed replication, the G-G breadth run, the G-H momentum-upgrade
run) are scripted, with the
env-variable file lists defined *and checked to exist* so the empty-list fallback (`BUGS.md` #69)
cannot recur:
...
reference, the expected readouts and what to send back.
...
| --- | ---: | --- |
| `sanity.test.js` | 60 | internal invariants: probability bounds, finite weights/gradients, **LSH index consistency**, determinism under a seeded PRNG, save/load fidelity, predicate edge cases, extreme inputs |
| `core.test.js` | 46 | end-to-end controller contract (signal shape, directional invariants, persistence, broadcast/translate) |
...
| `sample_weights.test.js` | 57 | **sample-uniqueness loss weighting** (`training/sample_weights.js`, Lopez de Prado AFML ch. 4): exact average uniqueness on fixed label intervals (bit-for-bit vs `analysis/uniqueness.js`), mean-1 / sum-1 normalisation, Kish ESS, `spanWeightsFromEntries` overlap semantics, and `train(inputs, target, w)` being exactly linear in `w` (energy ratio `w²`) with `w=1`/non-finite `w` a bit-exact no-op |
